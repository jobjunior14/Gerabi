...
export function Bralima ()
{
    ...
    //suiviStcok Information
    const  [displayDataMainExcel, setDisplayDataMainExcel] = useState(null) ;
    const  [displayTdSuivi, setDisplayTdSuivi] = useState (null);


    // Créez un objet Date avec la date actuelle


// Utilisez ces valeurs pour faire votre requête axios
        ...
        const fecthData = async () => {
             ...
             try {
                ...
                if ( year < todayYear || month < todayMonth || day < todayDay) {
                    ...
                    setBralimadata (prev => data.data.data.day.map ( (el, index) => {return {...el, id: index }} ));
                    setId( prev => data.data.data.id);
                    setUpdate (prev =>  true);
                    ...
                    if (previousData.data.data.day.length > 0) {
                        ...
                        setBralimadata (prev => previousData.data.data.day.map ((el, index) => {
                        ...
                        setBralimadata ( prev => previousData.data.data.day);
    ...
    function postData () {
        ...
        const fecthData = async () => {
            ...
            setBralimadata(null);
                ...
                setBralimadata (prev => response.data.data.day);
    ...
    function UpdateData () {
        ...
        const fecthData = async () => {
            ...
            try {
               ...
               setBralimadata (prev => response.data.data.day.map ( (el, index) => {return {...el, id: index }} ));
               setId(prev => response.data.data.id);
    ...
    //userEffect of suivi only to avoid searching to reMap suivi to levelUp performance 
    useEffect (() => {
        ...
        if (bralimaData && bralimaData.length > 0 ) {
                ...
                const mainData = bralimaData.map (
                ...
                setDisplayTdSuivi( tdData);

                setDisplayDataMainExcel ( mainData);
        }

    }, [bralimaData,toggleStoc, providers, readOnly, year, month, day ]);
