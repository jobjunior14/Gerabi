...
exports.getVenteDego = catchAssynch(async (req, res, next) => {
  let vente = await VenteDego.find();
...
exports.pushDataVente = catchAssynch(async (req, res, next) => {
  ...
  for (let o = 0; o < vente.length; o++) {
    const yearIndex = await vente[o].findIndex((el) => el.annee === year);

    if (yearIndex !== -1) {
      const monthIndex = await vente[o].data[yearIndex].data.findIndex(
        (el) => el.mois === month
      );

      if (monthIndex !== -1) {
        const dayIndex = await vente[o].data[yearIndex].data[
          monthIndex
        ].data.findIndex(
          (el) => Number(JSON.stringify(el.createdAt).slice(9, 11)) === day
        );

        if (dayIndex !== -1) {
          vente[o].data[yearIndex].data[monthIndex].data.push(req.body);
        }
      } else {
        vente[o].data[yearIndex].data.push({
          mois: month,
          data: req.body,
        });
      }
  ...
  for (let o = 0; o < vente.length; o++) {
    ...
    if (yearIndex !== -1) {
      ...
      if (monthIndex !== -1) {
        ...
        if (dayIndex !== -1) {
          vente[o].data[yearIndex].data[monthIndex].data[dayIndex] = {
            ...req.body,
