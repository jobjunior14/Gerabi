...
exports.getBralima = catchAssynch ( async (req, res, next) =>
{
    
    const features = new apiFeatures (Bralima.find(), req.body).filter();
...
exports.createBralima = catchAssynch ( async ( req, res, next) =>
{
    const newBralimaData = await Bralima.create(req.body);
...
exports.pushDataBralima = catchAssynch ( async (req, res, next) =>
{
    const bralima = await Bralima.findById (req.params.id );
...
exports.updateData = catchAssynch ( async ( req, res, next) =>
{
    const bralima = await Bralima.findById ( req.params.id1);
...
exports.getOneData = catchAssynch ( async  ( req, res, next) =>
{
    const bralimaOneData = await Bralima.findById( req.params.id );
...
{
    const stats = await Bralima.aggregate([
    { 
        $unwind: { path: "$stats"}
    },
    ]);
