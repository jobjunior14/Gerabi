...
exports.getBralima = catchAssynch ( async (req, res, next) =>
{
    ...
    let bralima = await Bralima.find();
...
exports.pushDataBralima = catchAssynch ( async (req, res, next) =>
{
    ...
    const bralima = await Bralima.find();
    ...
    if (bralima.length > 0 )
    {
        for ( o = 0; o < req.body.length; o++)
        {
            ...
            if (bralima[o].name.toUpperCase() === req.body[o].name.toUpperCase())
            {
                ...
                const newBralimaData = await Bralima.create(req.body[o]);
        ...
        for (let o of req.body)
        {
            ...
            const newBralimaData = await Bralima.create(o);
...
exports.updateDataBralima = catchAssynch ( async ( req, res, next) =>
{
    ...
    for (let i = 0; i < req.body.id.length; i++) {
        ...
        const bralima = await Bralima.findById( req.body.id[i]);
...
exports.getOneDataBralima = catchAssynch ( async  ( req, res, next) =>
{
...
exports.stastBralima  = catchAssynch ( async ( req, res, next) =>
{
    ...
    let stats = await Bralima.aggregate ([
        {
            $project:
            {
                stats:
