...
exports.getBrasimba = catchAssynch ( async (req, res, next) =>
{
    
    const features = new apiFeatures (Brasimba.find(), req.body).filter();
...
exports.createBrasimba = catchAssynch ( async ( req, res, next) =>
{
    const newBralimaData = await Brasimba.create(req.body);
...
exports.pushDataBrasimba = catchAssynch ( async (req, res, next) =>
{
    const bralima = await Brasimba.findById (req.params.id );
...
exports.updateDataBrasimba = catchAssynch ( async ( req, res, next) =>
{
    const bralima = await Brasimba.findById ( req.params.id1);
 ...
 /////////////////////////////////////recherche d'Id apres la modificaton du document///////////////////////////////////////////////////////////
                ...
                {
                    mois: statsObj1.mois,
                    vente_bar: Number (statsObj1.vente_bar) + Number (bralima.data[indexof].vente_journaliere.valeur) ,
                    approvionnement: Number (statsObj1.approvionnement) + Number ( bralima.data[indexof].benefice_sur_achat.val_gros_approvisionnement),
                    benefice: Number (statsObj1.benefice) + Number (bralima.data[indexof].benefice_sur_vente)
                };
...
exports.getOneDataBrasimba = catchAssynch ( async  ( req, res, next) =>
{
    const bralimaOneData = await Brasimba.findById( req.params.id );
...
{
    const stats = await Brasimba.aggregate([
    { 
        $unwind: { path: "$stats"}
    },
